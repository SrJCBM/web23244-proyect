...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  $stmt = $conexion->prepare("
    INSERT INTO opciones_categoria (id_categoria, nombre, tipo, modo_precio, valor_precio, obligatorio, visible, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
  ");
  $stmt->bind_param("issdiii", $id_categoria, $nombre, $tipo, $modo_precio, $valor, $obligatorio, $visible);
  $stmt->execute();
...
?>
<h3>➕ Crear opción</h3>
<form method="post" onsubmit="return confirm('¿Guardar opción?');">
  <label>Categoría<br>
  ...
  <label>Tipo<br>
    <select name="tipo"><option value="componente">componente</option><option value="accesorio">accesorio</option></select>
  </label><br><br>
  <label>Modo de precio<br>
    <select name="modo_precio"><option value="fijo">fijo</option><option value="porcentaje">porcentaje</option></select>
  </label><br><br>
  <label>Valor<br><input name="valor_precio" type="number" step="0.01" value="0"></label><br><br>
  ...
  <button type="button" onclick="cargarDirecto('administrador/componentes/lista_opciones.php<?= $id_cat?('?categoria='.$id_cat):'' ?>')">Cancelar</button>
</form>
