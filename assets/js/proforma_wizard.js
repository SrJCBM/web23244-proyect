...
window.prevPaso = function(n){ window.nextPaso(n); };

// ===== Paso 1: Cliente (global) =====
window.buscarClienteUI = async function(){
  const cedInput = document.getElementById('cliCedula');
  if (!cedInput) return;
  const ced = (cedInput.value || '').trim();
  if (!ced){ alert('Ingresa cédula'); return; }

  const r = await window.getJSON('cliente/cliente_clientes_buscar.php?cedula='+encodeURIComponent(ced));
  const box = document.getElementById('cliResultado');
  const nuevo = document.getElementById('cliNuevo');
  if (r && r.ok && r.found){
    const c = r.cliente; window.proforma.id_cliente = c.id_cliente;
    if (box) box.innerHTML = `<b>Cliente:</b> ${c.nombre_comercial} — ${c.correo||''} — ${c.telefono||''}`;
    if (nuevo) nuevo.style.display = 'none';
    const btn = document.getElementById('continuar1');
    if (btn) btn.disabled = false;
  } else {
    if (box) box.innerHTML = '<span style="color:#c00">No existe. Regístralo a la derecha.</span>';
...
window.registrarClienteUI = async function(){
  ...
  const r = await window.postJSON('cliente/cliente_clientes_crear.php', payload);
  if (r && r.ok){
...
// ===== Paso 2: Productos (global) =====
window.listarProductosUI = async function(){
  const sel = document.getElementById('selCategoria');
  if (!sel) return;
  const idc = +sel.value;
  const q = document.getElementById('prodQuery')?.value || '';
  window.proforma.id_categoria = idc;

  const data = await window.getJSON(`cliente/cliente_productos_listar.php?id_categoria=${idc}&q=${encodeURIComponent(q)}`);
  const cont = document.getElementById('listaProductos');
  if (!cont) return;

  cont.innerHTML = (data||[]).map(p=>{
    const checked = window.proforma.productos.find(x=>x.id_producto==p.id_producto) ? 'checked' : '';
    return `<label class="card">
      <input type="checkbox" ${checked} onchange="toggleProducto(${p.id_producto}, this.checked)">
      <b>${p.marca} ${p.modelo}</b> — ${p.nombre}
      <span class="badge">$${(+p.precio_base).toFixed(2)}</span>
    </label>`;
  }).join('');
};
...
window.toggleProducto = function(id, checked){
  if (checked){
    window.getJSON('cliente/cliente_producto_get.php?id='+id).then(p=>{
      if(window.proforma.id_categoria && p.id_categoria != window.proforma.id_categoria){
        alert('Solo productos de la misma categoría.'); window.listarProductosUI(); return;
      }
      if(!window.proforma.productos.find(x=>x.id_producto==id)){
        window.proforma.productos.push({...p, cantidad:1, items:[]});
      }
    });
  } else {
    window.proforma.productos = window.proforma.productos.filter(p=>p.id_producto!=id);
  }
};

window.validarProductos = function(){
  const n = window.proforma.productos.length;
  if (n < 2 || n > 4){ alert('Selecciona entre 2 y 4 productos.'); return; }
  window.cargarOpciones(); window.nextPaso(3);
};

// ===== Paso 3: Opciones (global) =====
window.cargarOpciones = async function(){
  const cont = document.getElementById('opcionsWrap');
  if (!cont) return;
  const opts = await window.getJSON('cliente/cliente_opciones_listar.php?id_categoria='+window.proforma.id_categoria);

  cont.innerHTML = window.proforma.productos.map((p,i)=>{
    const lista = (opts||[]).map(o=>{
      ...
      return `<label>
        ...
        <span class="badge">${o.modo_precio==='fijo' ? ('$'+(+o.valor_precio).toFixed(2)) : (o.valor_precio+'%')}</span>
      </label>`;
    }).join('<br>');
    ...
    if(o.obligatorio){
      window.proforma.productos.forEach(p=>{
        if(!p.items.find(it=>it.id_opcion==o.id_opcion)){
          p.items.push({ id_opcion:o.id_opcion, nombre:o.nombre, tipo:o.tipo, modo_precio:o.modo_precio, valor_precio:+o.valor_precio, cantidad:1 });
        }
      });
    }
  });
...
window.toggleItem = function(ev){
  ...
  window.getJSON('cliente/cliente_opciones_listar.php?id_categoria='+window.proforma.id_categoria).then(opts=>{
    const o = (opts||[]).find(x=>x.id_opcion==idop); if(!o) return;
    if(checked){
...
// ===== Cálculos / Resumen (global) =====
window.calcOpcion = function(p){
  const base = (+p.precio_base) * (p.cantidad||1);
  let extrasF=0, extrasPct=0;
  (p.items||[]).forEach(it=>{
    if(it.modo_precio==='fijo') extrasF += (+it.valor_precio) * (+it.cantidad||1);
...
window.armarResumen = function(){
  const cols = window.proforma.productos.map(p=>({ ...p, _calc: window.calcOpcion(p) }));
  let html = `<table class="table">
    <tr><th>Característica</th>${cols.map(p=>`<th>${p.marca} ${p.modelo}</th>`).join('')}</tr>
    <tr><td><b>Precio base</b></td>${cols.map(p=>`<td>$${(+p.precio_base).toFixed(2)}</td>`).join('')}</tr>
    <tr><td><b>Componentes/Accesorios</b></td>${
      cols.map(p=>`<td>${
        (p.items||[]).map(it=>`${it.nombre} <span class="badge">${it.modo_precio==='fijo' ? ('$'+(+it.valor_precio).toFixed(2)) : (it.valor_precio+'%')}</span>`).join('<br>')
      }</td>`).join('')
    }</tr>
    <tr><td><b>Extras</b></td>${cols.map(p=>`<td>$${p._calc.extras.toFixed(2)}</td>`).join('')}</tr>
    <tr><td><b>Subtotal</b></td>${cols.map(p=>`<td>$${p._calc.subtotal.toFixed(2)}</td>`).join('')}</tr>
  </table>`;
...
window.guardarProformaUI = async function(estado, enviar=false){
  ...
  if(!r || !r.ok){ alert((r && r.msg) || 'Error al guardar'); return; }

  if(estado==='emitida' || enviar){
    await fetch('cliente/proforma_pdf.php?id='+r.id_cotizacion, { credentials:'same-origin' });
  }
  if(enviar){
    const fd = new FormData(); fd.append('id', r.id_cotizacion);
    await fetch('cliente/proforma_enviar.php', { method:'POST', body:fd, credentials:'same-origin' });
    alert('Proforma enviada.');
  } else {
    alert('Proforma guardada.');
  }
  if (typeof cargarDirecto === 'function') cargarDirecto('cliente/historial_proformas.php');
};

// ===== Inicializador cuando el wizard está en el DOM =====
window.initProformaWizard = async function(){
  ...
  try {
    ...
    console.log('Categorias cargadas:', cats?.length || 0);

    // Si hay categorías, selecciona la primera real y lista productos
    if (cats && cats.length > 0) {
      sel.value = cats[0].id_categoria;
      if (typeof window.listarProductosUI === 'function') window.listarProductosUI();
    }
  } catch (e) {
